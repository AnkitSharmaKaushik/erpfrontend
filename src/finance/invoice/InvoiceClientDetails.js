...
const InvoiceClientDetails = ({ invoiceData, setInvoiceData }) => {
    const {clients} = useSelector((store)=>store.projectData)
      ...
      const currentClient = clients?.filter((c)=>c?.name?.toLowerCase()===selectedRecord?.clients?.toLowerCase())
      console.log("🚀 ~ InvoiceClientDetails ~ currentClient:", currentClient)
