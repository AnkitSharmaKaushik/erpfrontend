...
const SalesReport = (props) => {
  ...
  const loggedInUserDept =
    userList.length > 0 &&
    userList.find((user) => user.user_role.id == parseInt(userRole))?.department
      ?.id;
      ...
      <table className="min-w-full bg-white border border-gray-200 divide-y divide-gray-200 text-xs">
        ...
        <tbody className="divide-y divide-gray-200 text-center text-xs">
          {displayList.map((user, ind) => {
            ...
            const projectsUnderUser = filteredData.filter(
              (project) =>
                usersUnder.some(
                  (u) => project.created_by?.id == parseInt(u.user_role.id)
                ) || project.created_by?.id == user.id
            );
            const totalRevenue = projectsUnderUser.reduce((acc, curr) => {
              if (curr.cpi !== "0") {
                return acc + Number(curr.cpi) * Number(curr.sample);
              } else {
                return (
                  acc +
                  curr.project_samples.reduce((sum, item) => {
                    return sum + Number(item.cpi) * Number(item.sample);
                  }, 0)
                );
              }
            }, 0);
            ...
            const projectTypeData = projectsUnderUser.reduce((acc, project) => {
              ...
              acc[type].count += 1;
              acc[type].revenue += Number(project.cpi) * Number(project.sample);
              return acc;
            }, {});
